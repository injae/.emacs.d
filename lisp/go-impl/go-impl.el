...
(defvar go-impl--local-command-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map "TAB" #'go-impl--receiver-complete)
    map))
...
(defun go-impl--completing-function (input)
    ...
    (if (not (string-match "\\." input))
        ...
        (setq-local candidates (go-impl--collect-interfaces matched))))
    (cl-remove-if-not (lambda (it) (string-prefix-p input it)) candidates)
)

;;;###autoload
(defun go-impl (receiver)
    (interactive
